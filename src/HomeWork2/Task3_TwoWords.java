    ...
    public static void main(String[] args) {
        ...
        */
        ...
        if ((textWordOne.length()>0) && (textWordTwo.length() > 0) && isEven(textWordOne) && isEven(textWordTwo)) {
            String textResult;
            textResult = textWordOne.substring(0, textWordOne.length()/2)
                    + textWordTwo.substring(0, textWordTwo.length()/2);
            System.out.println("Result:" + textResult);
        } else{
            System.out.println("One of the words is uneven");
        }
    }
    public static boolean isEven(String textWord) { return (textWord.length() % 2 == 0); }
}
